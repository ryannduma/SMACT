...
def plot_electronegativity_vs_crystal_system(data, compound_type):
    ...
    sns.boxplot(x='crystal_system', y='electronegativity_difference', data=data[data['is_metal']], 
                color='lightblue', width=0.5, positions=np.arange(len(data['crystal_system'].unique()))-0.2)
    
    # Plot for non-metallic compounds
    sns.boxplot(x='crystal_system', y='electronegativity_difference', data=data[~data['is_metal']], 
                color='lightgreen', width=0.5, positions=np.arange(len(data['crystal_system'].unique()))+0.2)
...
def plot_electronegativity_vs_spacegroup(data, compound_type):
    ...
    # Plot for metallic compounds
    sns.scatterplot(x='spacegroup_number', y='electronegativity_difference', data=data[data['is_metal']], 
                    color='blue', alpha=0.5, label='Metallic')
    
    # Plot for non-metallic compounds
    sns.scatterplot(x='spacegroup_number', y='electronegativity_difference', data=data[~data['is_metal']], 
                    color='green', alpha=0.5, label='Non-metallic')
