...
def load_data(vec_file, compound_file):
    ...
    metal_data = vec_data[vec_data['is_metal'].astype(str).str.lower() == 'true']
    non_metal_data = vec_data[vec_data['is_metal'].astype(str).str.lower() == 'false']
    
    # Clean and convert VEC_count to numeric
    metal_data['VEC_count'] = pd.to_numeric(metal_data['VEC_count'], errors='coerce')
    non_metal_data['VEC_count'] = pd.to_numeric(non_metal_data['VEC_count'], errors='coerce')
    
    # Merge the data based on material_id
    metal_merged_data = pd.merge(metal_data, compound_data, on='material_id')
    non_metal_merged_data = pd.merge(non_metal_data, compound_data, on='material_id')
...
def plot_vec_vs_crystal_system(metal_data, non_metal_data, compound_type):
    ...
    sns.boxplot(x='crystal_system', y='VEC_count', data=non_metal_data, color='green')
...
def plot_vec_vs_spacegroup(metal_data, non_metal_data, compound_type):
    ...
    sns.scatterplot(x='spacegroup_number', y='VEC_count', data=metal_data, color='red', label='Metal')
    sns.scatterplot(x='spacegroup_number', y='VEC_count', data=non_metal_data, color='green', label='Non-Metal')
...
def plot_heatmap(metal_data, non_metal_data, compound_type):
    metal_pivot = pd.pivot_table(metal_data, values='VEC_count', index='crystal_system', columns='spacegroup_number', aggfunc='mean')
    non_metal_pivot = pd.pivot_table(non_metal_data, values='VEC_count', index='crystal_system', columns='spacegroup_number', aggfunc='mean')
